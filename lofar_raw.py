...
"""
This file was written by Pearse, quickes to use it here, but could do with re-writing/adding depending on needs.
"""
import copy
...
class LOFAR_Raw(LOFAR_BST):
	...
	def __init__(self, raw_file, sbs, obs_mode, trange=None, frange=None, bit_mode=8, dtype=np.float32, dt=5.12e-6):
		...
		self.obs_start = Time(raw_file.split("_")[4])
		self.__get_data()
	...
	def __get_data(self):
		if self.trange is not None:
			...
			count = int(count)
			data = np.memmap(self.raw_file, self.dtype, shape = count, offset=offset, mode="c").reshape(-1,488)
			
			
		else:
			data = np.memmap(self.raw_file, self.dtype, mode="c").reshape(-1,488)
